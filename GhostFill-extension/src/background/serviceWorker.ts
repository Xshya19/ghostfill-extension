...
// Email services - individual functions exported from module
import { setupContextMenu as contextMenuSetup } from '../background/contextMenu';
import { setupMessageHandler as messageHandlerSetup } from '../background/messageHandler';
...
import {
    emailService,
} from '../services/emailServices/index';

// OTP service - singleton instance
import { otpService } from '../services/otpService';

// Link service - singleton instance

// Polling manager - setup function
...
// Smart detection service - singleton instance (for warmup)
// Smart detection service - singleton instance (for warmup)
...
async function initLinkService(): Promise<void> {
    try {
        // Link service is already statically imported - no additional init needed
        log.debug('✅ Link service ready');
    } catch (error) {
        const errorMsg = error instanceof Error ? error.message : String(error);
        log.error('❌ Link service initialization failed', {
            error: errorMsg,
            stack: error instanceof Error ? error.stack : undefined,
        });
        throw error;
...
async function installKeepAlive(): Promise<void> {
    ...
    await chrome.alarms.clear(CONFIG.KEEPALIVE_ALARM_NAME).catch(() => { });
...
async function logStorageDiagnostics(): Promise<void> {
    ...
    await storageService.set(CONFIG.BOOT_STATE_KEY, {
        state: bootState,
...
async function executeBootSequence(): Promise<void> {
    ...
    if (bootErrors.length > 0) {
        ...
        const errorSummary = bootErrors.map((e, i) =>
...
async function executePhase(phase: PhaseDefinition): Promise<void> {
    ...
    await Promise.allSettled(
        phase.tasks.map((task) => executeTask(task, phase.order)),
    );
...
async function executeTask(task: TaskDefinition, phase: number): Promise<void> {
    ...
    for (let attempt = 0; attempt <= CONFIG.MAX_RETRIES_PER_TASK; attempt++) {
        ...
        try {
            ...
            await Promise.race([
                task.fn(),
                rejectAfter(CONFIG.TASK_TIMEOUT_MS, `Task "${task.name}" timed out after ${CONFIG.TASK_TIMEOUT_MS}ms`),
            ]);
...
function hasCriticalServiceDown(): boolean {
    for (const record of serviceRegistry.values()) {
        if (record.critical && record.health === 'down') { return true; }
    }
    return false;
}
...
export function getServiceHealth(): ReadonlyMap<string, Readonly<ServiceRecord>> {
    return new Map(
        Array.from(serviceRegistry.entries()).map(([k, v]) => [k, { ...v }]),
    );
}
...
self.addEventListener('error', (event: ErrorEvent) => {
    ...
    if (errorCount >= ERROR_THRESHOLD) {
        ...
        (async () => {
            try {
                // Check if storage is still accessible
                await storageService.getUsage();
                log.info('✅ Storage health check passed');

                // If we're in a degraded state, try to re-init
                if (bootState === 'degraded' || bootState === 'failed') {
                    log.info('🔄 Attempting service worker re-initialization');
                    await initServiceWorker();
                }
            } catch (recoveryError) {
                log.error('❌ Recovery failed', serializeError(recoveryError));
                // Extension is in an unrecoverable state
                // User will need to reload extension
            }
        })();
    }
});
...
self.addEventListener('unhandledrejection', (event: PromiseRejectionEvent) => {
    ...
    bootErrors.push({
        service: 'global',
        phase: -1,
        error: typeof reason === 'string' ? reason : ((reason as { message?: string }).message ?? String(event.reason)),
        timestamp: now,
        fatal: false,
    });
    ...
    if (errorCount >= ERROR_THRESHOLD) {
        ...
        (async () => {
            try {
                ...
                log.error('❌ Recovery from unhandled rejection failed');
            }
        })();
    }
});
