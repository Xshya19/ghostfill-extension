...
function registerDefaultActions(): void {
    ...
    register('smart-autofill', async (ctx) => {
        if (!ctx.tabId) { return { notifyType: 'error', notifyTitle: 'Error', notifyMessage: 'No active tab' }; }
    ...
    register('check-inbox', async () => {
        ...
        if (!currentEmail) {
            return { notifyType: 'error', notifyTitle: 'GhostFill: No Email', notifyMessage: 'Generate an email first' };
        }
...
function register(action: string, handler: ActionHandler): void {
    ...
    metrics.byAction.set(action, {
        clicks: 0, errors: 0, lastClickAt: 0, lastError: null,
    });
}
...
function safeUpdate(
    id: string,
    updates: chrome.contextMenus.UpdateProperties,
): void {
    try {
