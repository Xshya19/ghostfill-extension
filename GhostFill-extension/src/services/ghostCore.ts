...
const VERIFY_KEYWORDS = 'verification|security|confirmation|one[- ]?time|login|sign[- ]?in|auth|verify|confirm|validate|código|vérification|verificación|Bestätigung|パスワード|認証';
...
export function classifyWithGhostCore(
    ...
    if (linkResult.best && linkResult.best.score >= 20) {
        log.debug(`[LinkExtractor] Found optimal link: ${linkResult.best.url} (Score: ${linkResult.best.score})`);
    ...
    log.info(`[GhostCore] Classification complete in ${latency}ms: ${category} (C:${confidence.toFixed(2)})`);
...
function score(
    ...
    if (/expires?\s*in\s*\d+\s*(min|hour|second|minute)/i.test(text)) {codeScore += 15;}
    if (/don['’]?t\s*share|ne pas partager|no compartir/i.test(text)) {codeScore += 15;}
    if (new RegExp(`(?:${VERIFY_KEYWORDS})`).test(lowerText)) {codeScore += 10;}
    if (new RegExp(`(?:${CODE_KEYWORDS})`).test(lowerSubject)) {codeScore += 20;}
    ...
    if (/click\s*(the|this)?\s*(link|button|below)/i.test(text)) {linkScore += 15;}
    if (new RegExp(VERIFY_KEYWORDS).test(lowerSubject)) {linkScore += 15;}


    // ── 3. ANTI-SIGNALS (Phishing, Newsletters, Receipts) ──
    if (/newsletter|digest|roundup|update|trending|top\s*stories/i.test(lowerText)) {antiScore += 30;}
    if (/\b(sale|discount|%\s*off|promo|coupon|deal|buy\s*now|shop)\b/i.test(lowerText)) {antiScore += 30;}
    if (/order\s*(#|number)|receipt|invoice|payment|statement/i.test(lowerText)) {antiScore += 30;}
    if (/(liked|commented|mentioned|shared)\s*(your|you)/i.test(lowerText)) {antiScore += 25;}
    if (/tracking\s*number|shipped|delivered/i.test(lowerText)) {antiScore += 25;}
    ...
    // If the sender domain matches a tab the user is actively waiting on, 
    // it's highly likely this is the email we need.
    if (expectedDomains.length > 0) {
        let domainMatched = false;
        for (const expectedDomain of expectedDomains) {
            // Strip subdomains (e.g., mail.github.com -> github.com)
            const cleanExpected = expectedDomain.replace(/^www\./, '').toLowerCase();
            const cleanSender = lowerSender.split('@').pop()?.toLowerCase() || lowerSender;

            // Allow partial matching if the domains align closely (github.com in mail.github.com)
            if (cleanSender.includes(cleanExpected) || cleanExpected.includes(cleanSender)) {
                domainMatched = true;
                break;
            }

            // Heuristic for things like github.com vs github
            const rootExpected = cleanExpected.split('.')[0];
            const rootSender = cleanSender.split('.')[0];
            if (rootExpected === rootSender) {
                domainMatched = true;
                break;
            }
        }
