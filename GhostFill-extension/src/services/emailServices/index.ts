...
class EmailServiceAggregator {
    private availableServices: EmailService[] = ['maildrop', 'mailgw', 'mailtm', 'tmailor', 'guerrilla', 'tempmail', 'custom', '1secmail'];
    ...
    private async loadHealthState(): Promise<void> {
        if (this.healthCheckInitialized) { return; }

        try {
            const healthState = await storageService.get('emailServiceHealth');
            if (healthState && typeof healthState === 'object' && 'timestamp' in healthState && 'availableServices' in healthState) {
    ...
    async performHealthCheck(): Promise<void> {
        ...
        const checks = this.availableServices.map(async (service) => {
            ...
            if (!this.healthManager.isAvailable(service)) { return null; } // Skip unavailable providers
        ...
        // PERFORMANCE FIX: Persist health check results
    ...
    async generateEmail(
        ...
        return this.finalizeEmailGeneration(account, service, startTime);
    }

    /**
     * Create account with specified service
    ...
    private async createAccountWithService(
        ...
        switch (service) {
            case 'custom':
                return customDomainService.createAccount();
            case 'maildrop':
                return maildropService.createAccount(options.prefix);
            case 'tmailor':
                return tmailorService.createAccount(options.prefix);
            case 'templol':
                log.warn('TempMailLol unavailable (403), using TMailor');
                return tmailorService.createAccount(options.prefix);
            case 'mailgw':
                return mailGwService.createAccount(options.prefix);
            case 'mailtm':
                try {
                    return await mailTmService.createAccount(options.prefix);
                } catch (error) {
                    log.warn('Mail.tm failed, falling back to TMailor', error);
                    return tmailorService.createAccount(options.prefix);
                }
            case 'guerrilla':
                return guerrillaMailService.createAccount();
            case 'dropmail':
                log.warn('DropMail unavailable, using TMailor');
                return tmailorService.createAccount(options.prefix);
            case 'tempmail':
            case '1secmail':
                return tempMailService.generateEmail(options.prefix, options.domain);
            default:
                log.warn(`Unknown service "${service}", defaulting to Maildrop`);
                return maildropService.createAccount(options.prefix);
        }
    }
    ...
    private async generateEmailWithFallback(
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            ...
            log.info(`Retry ${attempt + 1}/${maxRetries}: Trying ${nextProvider} in ${Math.round(delay)}ms`);
            await new Promise(resolve => { setTimeout(resolve, delay); });

            try {
                const account = await this.createAccountWithService(nextProvider, options);
                return this.finalizeEmailGeneration(account, nextProvider, startTime);
            } catch (error) {
    ...
    private async finalizeEmailGeneration(
        ...
        log.info('Email generated', { email: account.fullEmail, service, responseTime: `${Math.round(responseTime)}ms` });
        return account;
    }
    ...
    async checkInbox(account: EmailAccount): Promise<Email[]> {
        try {
            ...
            if (isNetworkError) {
                ...
                log.warn('Failed to check inbox (will retry)', { service: account.service, error: errorMsg });
            }
