...
const EMAIL_INTENT_SIGNALS: IntentSignal[] = [
    // ── Verification ─────────────────────────────────────────
    { pattern: /verify\s*(your|my|this|the)?\s*(email|e-?mail|account|address|identity)/i, intent: 'verification', weight: 40, name: 'verify-email' },
    { pattern: /confirm\s*(your|my|this|the)?\s*(email|e-?mail|account|registration|sign\s*up)/i, intent: 'verification', weight: 40, name: 'confirm-email' },
    { pattern: /activate\s*(your|my|this)?\s*(account|email|profile)/i, intent: 'verification', weight: 35, name: 'activate' },
    ...
    { pattern: /verification\s*code/i, intent: 'verification', weight: 45, name: 'verification-code-phrase' },
    { pattern: /confirmation\s*code/i, intent: 'verification', weight: 45, name: 'confirmation-code-phrase' },
    { pattern: /one[- ]?time\s*(code|password|passcode|pin)/i, intent: 'verification', weight: 50, name: 'one-time-code' },
    ...
    { pattern: /thanks?\s*(for)?\s*(signing\s*up|registering|joining)/i, intent: 'verification', weight: 20, name: 'thanks-signup' },
    ...
    // ── Two-Factor ───────────────────────────────────────────
    { pattern: /two[- ]?factor|2fa|multi[- ]?factor|mfa/i, intent: 'two-factor', weight: 45, name: '2fa' },
    ...
    { pattern: /forgot\s*(your|my)?\s*password/i, intent: 'password-reset', weight: 40, name: 'forgot-password' },
    { pattern: /password\s*(recovery|reset|change)/i, intent: 'password-reset', weight: 40, name: 'password-recovery' },
    { pattern: /change\s*(your|my)?\s*password/i, intent: 'password-reset', weight: 35, name: 'change-password' },
    { pattern: /password\s*reset\s*code/i, intent: 'password-reset', weight: 50, name: 'password-reset-code' },

    // ── Magic Link Login ─────────────────────────────────────
    { pattern: /magic\s*link/i, intent: 'magic-link-login', weight: 45, name: 'magic-link' },
    { pattern: /sign\s*in\s*(to|with|using)\s*(your|a|this)?\s*(link|button)/i, intent: 'magic-link-login', weight: 40, name: 'sign-in-link' },
    { pattern: /passwordless/i, intent: 'magic-link-login', weight: 40, name: 'passwordless' },

    // ── Device Confirmation ──────────────────────────────────
    { pattern: /new\s*(device|browser|location|login|sign\s*in)/i, intent: 'device-confirmation', weight: 35, name: 'new-device' },
    { pattern: /was\s*this\s*you/i, intent: 'device-confirmation', weight: 30, name: 'was-this-you' },
    { pattern: /unusual\s*(activity|sign\s*in|login)/i, intent: 'device-confirmation', weight: 30, name: 'unusual-activity' },

    // ── Anti-Intents ─────────────────────────────────────────
    { pattern: /\b(sale|discount|%\s*off|promo|coupon|deal|buy\s*now|shop\s*now)\b/i, intent: 'marketing', weight: 40, name: 'marketing' },
    { pattern: /newsletter|weekly\s*(digest|recap|update)|monthly\s*(digest|recap)/i, intent: 'newsletter', weight: 45, name: 'newsletter' },
    { pattern: /order\s*(confirm|#|number)|receipt|invoice/i, intent: 'transactional', weight: 40, name: 'transactional' },
    { pattern: /ship(ped|ping)|delivered|tracking\s*(number|#)/i, intent: 'transactional', weight: 40, name: 'shipping' },
    { pattern: /(liked|commented|mentioned|followed|shared|replied)\s*(your|you)/i, intent: 'social-notification', weight: 35, name: 'social' },
    { pattern: /payment\s*(received|confirmed|processed)/i, intent: 'transactional', weight: 35, name: 'payment' },
];
...
function matchOTPProvider(
    ...
    for (const provider of PROVIDER_OTP_KNOWLEDGE) {
        ...
        const bodyMatch = provider.senderPatterns.some(p => p.test(bodyText));
...
const CODE_LABEL_PATTERNS: Array<{
    ...
    codeType: CodeType;
        ...
        {
            regex: /(?:your|the|a)?\s*(?:one[- ]?time)\s*(?:pass(?:word|code)|code|pin|otp)\s*(?:is|:|-|–|—)\s*/gi,
            weight: 55,
            name: 'one-time-code-label',
            codeType: 'otp',
        },
        {
            regex: /(?:your|the|a)?\s*(?:verification|security|confirmation|authentication|login|sign[- ]?in|access)\s*(?:code|pin|otp|number)\s*(?:is|:|-|–|—)\s*/gi,
            weight: 50,
        ...
        {
            regex: /(?:enter|use|type|input|paste|submit)\s*(?:the|this|your)?\s*(?:following\s*)?(?:code|pin|otp|number)\s*(?:to|:|-|–|—)?\s*/gi,
            weight: 40,
...
const CODE_POSTFIX_PATTERNS: Array<{
    ...
    name: string;
        ...
        { regex: /\s*(?:is\s*)?(?:your|the)\s*(?:verification|security|confirmation|authentication|login|one[- ]?time)?\s*(?:code|pin|otp|passcode|password)/i, weight: 45, name: 'postfix-is-your-code' },
        { regex: /\s*\.\s*(?:enter|use|type)\s*(?:this|it)\s*(?:to|in|on|at)/i, weight: 30, name: 'postfix-enter-to' }
    ];
...
function extractRawCandidates(
    ...
    for (const labelPattern of CODE_LABEL_PATTERNS) {
        if (labelPattern.weight === 0) {continue;} // Skip placeholder
        ...
        while ((match = regex.exec(plainText)) !== null) {
            ...
            if (alphaMatch && /\d/.test(alphaMatch[1]) && /[A-Za-z]/.test(alphaMatch[1]) && !seenValues.has(alphaMatch[1])) {
    ...
    const actionRegex = /(?:enter|use|type|input|paste|submit)\s+(\d{4,8})\s+(?:to|in|on|at|for|into|below|above|here)/gi;
    ...
    for (const numMatch of standaloneNumbers) {
        ...
        const afterCode = plainText.slice(numMatch.index! + code.length, numMatch.index! + code.length + 100);
    ...
    if (doc) { // Only run if doc is available
    ...
    while ((urlMatch = urlRegex.exec(htmlBody)) !== null) {
        ...
        const CODE_URL_PARAMS = [
            ...
            'confirmation_code', 'confirmation-code', 'confirmationCode',
            'otp', 'code', 'pin',
        ...
        try {
            ...
            for (const [key, val] of urlObj.searchParams) {
                if (CODE_URL_PARAMS.includes(key)) {continue;} // Already checked
                if (/code|otp|pin|token/i.test(key) && val.length >= 4 && val.length <= 10 && /^[A-Za-z0-9]+$/.test(val)) {
                    if (!seenValues.has(val)) {
                        seenValues.add(val);
                        candidates.push({
                            value: val,
    ...
    for (const numMatch of allNumbers) {
        ...
        const hasVerificationContext =
            /verif|confirm|authenti|one[- ]?time|otp|security\s*code|login\s*code|sign[- ]?in\s*code|passcode/i.test(lowerCtx);
...
const CONTEXT_POSITIVE_PATTERNS: Array<{
    ...
    codeType: CodeType | null;
        ...
        { pattern: /(?:your|the|a)\s*(?:verification|security|confirmation|authentication)\s*code/i, score: 25, name: 'ctx:verification-code-phrase', codeType: 'verification-code' },
        { pattern: /one[- ]?time\s*(?:code|password|passcode|pin|otp)/i, score: 30, name: 'ctx:one-time-code', codeType: 'otp' },
        { pattern: /(?:login|sign[- ]?in|access)\s*code/i, score: 25, name: 'ctx:login-code', codeType: 'login-code' },
        ...
        // ── Expiration / urgency ────────────────────────────────
        { pattern: /expires?\s*(in|after)\s*\d+\s*(min|hour|second|minute)/i, score: 20, name: 'ctx:expires-in', codeType: null },
        ...
        { pattern: /this\s*code\s*(will\s*)?expire/i, score: 18, name: 'ctx:code-will-expire', codeType: null },
        { pattern: /time[- ]?sensitive/i, score: 10, name: 'ctx:time-sensitive', codeType: null },

        // ── Security / don't share ──────────────────────────────
        { pattern: /don['']?t\s*share\s*(this|the|your)?\s*(code|number|otp|pin)/i, score: 20, name: 'ctx:dont-share-code', codeType: null },
        { pattern: /keep\s*(this|it)?\s*(code|number)?\s*(?:private|secret|safe|secure)/i, score: 15, name: 'ctx:keep-secret', codeType: null },
        ...
        // ── Action instructions ─────────────────────────────────
        { pattern: /enter\s*(this|the|your)?\s*(code|number|otp|pin)/i, score: 20, name: 'ctx:enter-code', codeType: null },
        { pattern: /paste\s*(this|the|your)?\s*(code|number)/i, score: 18, name: 'ctx:paste-code', codeType: null },
        { pattern: /type\s*(this|the|your)?\s*(code|number)/i, score: 18, name: 'ctx:type-code', codeType: null },
        { pattern: /use\s*(this|the|your)?\s*(code|number)/i, score: 15, name: 'ctx:use-code', codeType: null },
        { pattern: /on\s*the\s*(?:verification|login|sign[- ]?in|confirmation)\s*(?:page|screen|form)/i, score: 12, name: 'ctx:on-verification-page', codeType: null },
        { pattern: /(?:in|into)\s*the\s*(?:field|input|box|form)/i, score: 10, name: 'ctx:into-field', codeType: null },

        // ── Disclaimer ──────────────────────────────────────────
        { pattern: /if\s*you\s*didn['']?t\s*(?:request|create|initiate)/i, score: 15, name: 'ctx:didnt-request', codeType: null },
        { pattern: /ignore\s*this\s*(?:email|message)/i, score: 12, name: 'ctx:ignore-email', codeType: null },
    ];
...
const CONTEXT_NEGATIVE_PATTERNS: Array<{
    ...
    name: string;
        ...
        // ── Phone numbers ───────────────────────────────────────
        { pattern: /(?:phone|call|tel|mobile|fax|dial|contact|reach)\s*(?:us|me|at)?/i, score: -40, name: 'ctx:phone-context' },
        ...
        { pattern: /(?:street|ave|avenue|blvd|boulevard|road|rd|drive|dr|lane|ln|way|suite|ste|floor|apt)\b/i, score: -35, name: 'ctx:street-address' },
        { pattern: /(?:city|state|province|country)\s*:/i, score: -30, name: 'ctx:address-field' },

        // ── Money / prices ──────────────────────────────────────
        { pattern: /[$€£¥₹]\s*\d/i, score: -45, name: 'ctx:currency-symbol' },
        { pattern: /(?:price|cost|total|amount|balance|charge|fee|payment|paid|usd|eur|gbp)\s*:?\s*/i, score: -40, name: 'ctx:money-word' },
        { pattern: /\d+\.\d{2}\b/i, score: -25, name: 'ctx:decimal-amount' },

        // ── Order / transaction IDs ─────────────────────────────
        { pattern: /(?:order|transaction|reference|invoice|receipt|booking|reservation|confirmation)\s*(?:#|no\.?|number|id|ref)\s*:?\s*/i, score: -50, name: 'ctx:order-id' },
        { pattern: /(?:#|no\.?)\s*:?\s*$/i, score: -25, name: 'ctx:hash-number-prefix' },
        { pattern: /(?:tracking|shipment|package|parcel)\s*(?:#|no\.?|number|id)/i, score: -45, name: 'ctx:tracking-number' },

        // ── Account / user IDs ──────────────────────────────────
        { pattern: /(?:account|user|member|customer|employee|staff|badge|ticket)\s*(?:#|no\.?|number|id)\s*:?\s*/i, score: -40, name: 'ctx:account-id' },
        { pattern: /(?:id|identifier)\s*:?\s*$/i, score: -20, name: 'ctx:id-suffix' },

        // ── Dates / times ───────────────────────────────────────
        { pattern: /(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*\s*\d/i, score: -30, name: 'ctx:date-month' },
        { pattern: /\d{1,2}[/:]\d{1,2}[/:]\d{2,4}/i, score: -35, name: 'ctx:date-format' },
        { pattern: /(?:am|pm)\b/i, score: -20, name: 'ctx:time-ampm' },
        { pattern: /(?:minutes?|hours?|days?|seconds?|weeks?)\s*(?:ago|later|from\s*now)/i, score: -15, name: 'ctx:relative-time' },
        ...
        { pattern: /(?:step|page|chapter|section|item|row|col|column)\s*\d/i, score: -25, name: 'ctx:enumeration' },
        { pattern: /(?:kb|mb|gb|tb|px|em|rem|pt|hz|mhz|ghz)\b/i, score: -25, name: 'ctx:unit' },

        // ── Marketing ───────────────────────────────────────────
        { pattern: /(?:promo(?:tion)?|coupon|voucher|discount|gift)\s*code/i, score: -50, name: 'ctx:promo-code' },
...
function analyzeContext(candidate: RawCandidate): ContextAnalysis {
    ...
    for (const pattern of CONTEXT_POSITIVE_PATTERNS) {
        if (pattern.pattern.test(context)) {
            ...
            if (pattern.codeType && !detectedType) {detectedType = pattern.codeType;}
            if (pattern.name.includes('expire') || pattern.name.includes('valid') || pattern.name.includes('within') || pattern.name.includes('time')) {
                hasExpiration = true;
            }
            if (pattern.name.includes('dont-share') || pattern.name.includes('keep-secret') || pattern.name.includes('never-share')) {
                hasDontShare = true;
            }
            if (pattern.name.includes('enter') || pattern.name.includes('paste') || pattern.name.includes('type') || pattern.name.includes('use')) {
                hasInstruction = true;
            }
        }
    }
...
function checkAntiPatterns(
    ...
    if (cleanCode.length === 4 && num >= 1900 && num <= 2099) {
        ...
        const yearContext = /copyright|©|since|founded|established|year|\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*\s*\d{1,2}/i.test(context);
        if (yearContext) {
            signals.push({ name: 'anti:year-with-context', points: -80, layer: 'anti-pattern', detail: `${cleanCode} looks like a year in this context` });
            return { isRejected: true, reason: `year:${cleanCode} `, penaltyScore: -80, signals };
        }
        // Even without context, 4-digit years are suspicious
        penalty -= 20;
        signals.push({ name: 'anti:possible-year', points: -20, layer: 'anti-pattern', detail: `${cleanCode} could be a year(penalized but not rejected)` });
    }
    ...
    if (cleanCode.length >= 7 && cleanCode.length <= 10) {
        if (/phone|call|tel|mobile|contact|dial|fax|reach|whatsapp|sms|text\s*us/i.test(context)) {
            signals.push({ name: 'anti:phone-number', points: -90, layer: 'anti-pattern', detail: `${cleanCode} is in phone context` });
    ...
    if (/^1?8[0-9]{2}/.test(cleanCode) && cleanCode.length >= 7) {
        signals.push({ name: 'anti:toll-free-pattern', points: -60, layer: 'anti-pattern', detail: `${cleanCode} starts with toll - free prefix` });
        return { isRejected: true, reason: `toll - free:${cleanCode} `, penaltyScore: -60, signals };
    }

    // ── Zip Code Detection ──────────────────────────────────
    if (cleanCode.length === 5 && /zip|postal|address|city|state|avenue|street|blvd|suite/i.test(context)) {
        signals.push({ name: 'anti:zip-code', points: -80, layer: 'anti-pattern', detail: `${cleanCode} is in address context` });
        return { isRejected: true, reason: `zip:${cleanCode} `, penaltyScore: -80, signals };
    }

    // ── Price / Amount ──────────────────────────────────────
    if (/[$€£¥₹]\s*$/.test(context.slice(0, context.indexOf(code))) ||
        /^\s*[$€£¥₹]/.test(context.slice(context.indexOf(code) + code.length))) {
        signals.push({ name: 'anti:price', points: -80, layer: 'anti-pattern', detail: `${cleanCode} is near currency symbol` });
        return { isRejected: true, reason: `price:${cleanCode} `, penaltyScore: -80, signals };
    }
    if (/price|cost|total|amount|balance|charge|fee|paid|payment|subtotal|tax|shipping\s*cost/i.test(context)) {
        penalty -= 40;
        signals.push({ name: 'anti:money-context', points: -40, layer: 'anti-pattern', detail: `${cleanCode} is in money context` });
    }

    // ── Order / Reference Number ────────────────────────────
    if (/order\s*(#|no|number|id)|reference\s*(#|no|number|id)|invoice\s*(#|no|number)|receipt|booking|reservation|confirmation\s*(#|no|number)/i.test(context)) {
        ...
        if (!/confirmation\s*code|verification\s*code|security\s*code/i.test(context)) {
            penalty -= 50;
            signals.push({ name: 'anti:order-number', points: -50, layer: 'anti-pattern', detail: `${cleanCode} is in order / reference context` });
        }
    }

    // ── Tracking Number ─────────────────────────────────────
    if (/tracking|shipment|package|parcel|carrier|fedex|ups|usps|dhl/i.test(context)) {
        signals.push({ name: 'anti:tracking-number', points: -70, layer: 'anti-pattern', detail: `${cleanCode} is in shipping context` });
        return { isRejected: true, reason: `tracking:${cleanCode} `, penaltyScore: -70, signals };
    }

    // ── Account ID ──────────────────────────────────────────
    if (/account\s*(#|no|number|id)|user\s*(#|id)|member\s*(#|id)|customer\s*(#|id)|employee\s*(#|id)/i.test(context)) {
        if (!/verification|security|confirmation|one[- ]?time/i.test(context)) {
            penalty -= 45;
            signals.push({ name: 'anti:account-id', points: -45, layer: 'anti-pattern', detail: `${cleanCode} is in account ID context` });
        }
    }

    // ── Time Duration (the "10" in "expires in 10 minutes") ──
    if (/expires?\s*(in|after)\s*$/.test(context.slice(0, context.indexOf(code)).trim()) ||
        /^\s*(minutes?|hours?|seconds?|days?)\b/i.test(context.slice(context.indexOf(code) + code.length))) {
        if (num <= 60) {
            signals.push({ name: 'anti:time-duration', points: -70, layer: 'anti-pattern', detail: `${cleanCode} is a time duration` });
    ...
    if (cleanCode.length <= 3) {
        signals.push({ name: 'anti:too-short', points: -60, layer: 'anti-pattern', detail: `${cleanCode} is too short for an OTP` });
    ...
    if (cleanCode.length >= 9 && /^\d+$/.test(cleanCode)) {
        penalty -= 30;
        signals.push({ name: 'anti:long-number', points: -30, layer: 'anti-pattern', detail: `${cleanCode} is unusually long for an OTP(${cleanCode.length} digits)` });
    }

    // ── Promo / coupon code context ─────────────────────────
    if (/promo|coupon|voucher|discount|gift\s*card|referral/i.test(context)) {
        signals.push({ name: 'anti:promo-code', points: -70, layer: 'anti-pattern', detail: `${cleanCode} is in promotional context` });
    ...
    if (/^(\d)\1+$/.test(cleanCode)) {
        penalty -= 25;
        signals.push({ name: 'anti:repeated-digits', points: -25, layer: 'anti-pattern', detail: `${cleanCode} is all the same digit` });
    }

    // ── Sequential digits (123456, 654321) ──────────────────
    const isSequential = cleanCode.split('').every((d, i, arr) =>
        i === 0 || parseInt(d, 10) === parseInt(arr[i - 1], 10) + 1
    );
    const isReverseSequential = cleanCode.split('').every((d, i, arr) =>
        i === 0 || parseInt(d, 10) === parseInt(arr[i - 1], 10) - 1
    );
    if (isSequential || isReverseSequential) {
        penalty -= 15;
        signals.push({ name: 'anti:sequential', points: -15, layer: 'anti-pattern', detail: `${cleanCode} is sequential(slightly suspicious)` });
    }
...
function analyzeVisualProminence(candidate: RawCandidate, doc: Document | null): {
    ...
    if (!candidate.fromHtml || !candidate.htmlElement) {
        ...
        if (!foundElement) {return { prominenceScore: 0, signals: [] };}
    ...
    if (tag === 'strong' || tag === 'b' || parentTag === 'strong' || parentTag === 'b') {
        score += 20;
        signals.push({ name: 'visual:bold', points: 20, layer: 'visual-prominence', detail: 'Code is in bold text' });
    }
    if (tag === 'code' || tag === 'pre' || parentTag === 'code') {
        score += 15;
        signals.push({ name: 'visual:code-tag', points: 15, layer: 'visual-prominence', detail: 'Code is in <code> or <pre> tag' });
    }
    if (/^h[1-3]$/.test(tag) || /^h[1-3]$/.test(parentTag)) {
        score += 25;
        signals.push({ name: 'visual:heading', points: 25, layer: 'visual-prominence', detail: `Code is in <${tag || parentTag}> heading` });
    }
    ...
    if (fontSizeMatch) {
        ...
        if (fontSize >= 24) {
            score += 25;
            signals.push({ name: 'visual:large-font', points: 25, layer: 'visual-prominence', detail: `font - size: ${fontSize} px` });
        } else if (fontSize >= 18) {
            score += 15;
            signals.push({ name: 'visual:medium-font', points: 15, layer: 'visual-prominence', detail: `font - size: ${fontSize} px` });
        }
    }
    ...
    if (/font-weight\s*:\s*(bold|700|800|900)/i.test(style)) {
        score += 12;
        signals.push({ name: 'visual:font-weight-bold', points: 12, layer: 'visual-prominence', detail: 'Code has bold font-weight' });
    }
    ...
    if (/letter-spacing\s*:\s*(\d+)/i.test(style)) {
        ...
        if (spacing >= 2) {
            score += 20;
            signals.push({ name: 'visual:letter-spacing', points: 20, layer: 'visual-prominence', detail: `letter - spacing: ${spacing} px(OTP display style)` });
        }
    }
    ...
    if (/text-align\s*:\s*center/i.test(style) ||
        ...
        score += 10;
        signals.push({ name: 'visual:centered', points: 10, layer: 'visual-prominence', detail: 'Code is centered' });
    }

    // Monospace font
    if (/font-family\s*:.*(?:mono|courier|consolas|menlo|roboto\s*mono)/i.test(style)) {
        score += 15;
        signals.push({ name: 'visual:monospace', points: 15, layer: 'visual-prominence', detail: 'Code uses monospace font' });
    }
    ...
    if (colorMatch) {
        ...
        if (color !== 'black' && color !== '#000' && color !== '#000000' &&
            ...
            color !== '#333' && color !== '#333333' && color !== 'rgb(0, 0, 0)') {
            score += 8;
            signals.push({ name: 'visual:colored', points: 8, layer: 'visual-prominence', detail: `Code has custom color: ${color} ` });
        }
    }

    // Background color (code highlight box)
    if (/background(?:-color)?\s*:\s*(?!transparent|none|inherit|initial|#fff|white|rgb\(255)/i.test(style)) {
        score += 15;
        signals.push({ name: 'visual:background', points: 15, layer: 'visual-prominence', detail: 'Code has background highlight' });
    }

    // Padding (styled code box)
    if (/padding\s*:\s*\d+/i.test(style)) {
        score += 8;
        signals.push({ name: 'visual:padded', points: 8, layer: 'visual-prominence', detail: 'Code has padding (box style)' });
    }

    // Border (code in a bordered box)
    if (/border\s*:/i.test(style) && !/border\s*:\s*none/i.test(style)) {
        score += 10;
        signals.push({ name: 'visual:bordered', points: 10, layer: 'visual-prominence', detail: 'Code has a border' });
    }
    ...
    if (elText === candidate.rawValue || elText === candidate.value) {
        score += 15;
        signals.push({ name: 'visual:isolated-code', points: 15, layer: 'visual-prominence', detail: 'Element contains only the code (no other text)' });
    }
...
function validateFormat(code: string, rawCode: string): {
    format: CodeFormat;
    length: number;
    ...
    if (len === 6) {
        formatScore += 15;
        signals.push({ name: 'format:ideal-length-6', points: 15, layer: 'format-validator', detail: '6-digit code (most common OTP length)' });
    } else if (len === 4 || len === 5) {
        formatScore += 10;
        signals.push({ name: `format: common - length - ${len} `, points: 10, layer: 'format-validator', detail: `${len} -digit code(common OTP length)` });
    } else if (len === 7 || len === 8) {
        formatScore += 8;
        signals.push({ name: `format: valid - length - ${len} `, points: 8, layer: 'format-validator', detail: `${len} -digit code(valid OTP length)` });
    } else if (len < 4) {
        formatScore -= 30;
        signals.push({ name: 'format:too-short', points: -30, layer: 'format-validator', detail: `${len} digits is too short for an OTP` });
    } else if (len > 8 && format === 'numeric') {
        formatScore -= 15;
        signals.push({ name: 'format:too-long', points: -15, layer: 'format-validator', detail: `${len} digits is unusually long for a numeric OTP` });
    }

    // Format scoring
    if (format === 'numeric') {
        formatScore += 5;
        signals.push({ name: 'format:numeric', points: 5, layer: 'format-validator', detail: 'Pure numeric format (most common)' });
    } else if (format === 'alphanumeric') {
        formatScore += 3;
        signals.push({ name: 'format:alphanumeric', points: 3, layer: 'format-validator', detail: 'Alphanumeric format' });
    } else if (format === 'dash-separated') {
        formatScore += 8;
        signals.push({ name: 'format:dash-separated', points: 8, layer: 'format-validator', detail: 'Dash-separated format (common display style)' });
    }
...
function buildOTPReasoning(
    ...
    // ── Email intent ────────────────────────────────────────
    const verificationIntents: EmailIntent[] = ['verification', 'two-factor', 'password-reset', 'device-confirmation'];
    ...
    steps.push({
        ...
        conclusion: candidate.strategy === 'explicit-label'
        ...
        impact: candidate.strategyScore >= 40 ? 'strong-positive' : candidate.strategyScore >= 25 ? 'positive' : 'neutral',
    });

    // ── Context ─────────────────────────────────────────────
    if (contextAnalysis.contextScore > 20) {
        const details: string[] = [];
        if (contextAnalysis.hasExpiration) {details.push('expiration language');}
        if (contextAnalysis.hasDontShare) {details.push('"don\'t share" warning');}
        if (contextAnalysis.hasInstruction) {details.push('entry instruction');}
    ...
    if (totalScore >= 80) {
        ...
        confidenceExplanation = 'Multiple independent layers strongly agree this is the verification code.';
...
export function extractOTP(
    ...
    for (const p of codeWordPatterns) {
        if (p.regex.test(fullText)) { codeWordUsed = p.word; break; }
    }
    ...
    for (const raw of rawCandidates) {
        ...
        if (verificationIntents.includes(intentResult.intent)) {confidence = Math.min(confidence * 1.1, 1.0);}
        if (providerMatch && isProviderExpectedLength(raw.value, providerMatch)) {confidence = Math.min(confidence * 1.1, 1.0);}
        if (raw.strategy === 'explicit-label') {confidence = Math.min(confidence * 1.1, 1.0);}
        ...
        if (codeType === 'unknown-code' && providerMatch) {
            ...
            if (prov) {codeType = prov.codeType;}
        }
        ...
        const reasoning = buildOTPReasoning(
            raw, contextAnalysis, visualResult.prominenceScore,
            formatResult, antiResult, intentResult.intent,
            providerMatch, totalScore,
        );
        ...
        candidates.push({
            ...
            urlParam: raw.strategy === 'url-parameter' ? (raw.label?.replace('URL param: ', '') || null) : null,
    ...
    // ── Email Analysis ──────────────────────────────────────
    const hasExpirationLanguage = /expires?\s*(in|after)|valid\s*(for|until)|within\s*\d+/i.test(plainText);
    const hasSecurityLanguage = /don['']?t\s*share|keep\s*(it)?\s*secret|security|suspicious|unauthorized/i.test(plainText);
    const hasDontShareLanguage = /don['']?t\s*share|never\s*(share|give|send)|keep\s*(it)?\s*(private|secret)/i.test(plainText);
    ...
    if (lengthHint) {estimatedCodeLength = parseInt(lengthHint[1], 10);}
...
function getElementContext(el: Element, radius: number): string {
    ...
    for (let i = 0; i < 2 && prev; i++) {
        ...
        if (t.length > 0 && t.length < 200) {parts.unshift(t);}
        prev = prev.previousElementSibling;
    }

    // Parent text
    const parentText = getDirectText(el.parentElement);
    if (parentText.length > 0) {parts.push(parentText);}
