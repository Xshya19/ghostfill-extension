...
const CODE_PARAM_NAMES = [
    'code', 'token', 'otp', 'verification_code', 'verificationCode',
    'verify', 'key', 'pin', 'confirmation_code', 'confirmationCode',
    'auth_code', 'authCode', 'passcode', 'secret', 'vcode',
] as const;

// ── Path prefixes that precede an inline code (e.g. /verify/ABC123) ──
const CODE_PATH_PREFIXES = [
    '/verify/', '/confirm/', '/activate/', '/validation/',
    '/auth/', '/code/', '/token/', '/check/',
] as const;

// ── Blocked URL schemes ──
const BLOCKED_SCHEMES: ReadonlySet<string> = new Set([
    'javascript:', 'data:', 'blob:', 'file:', 'chrome:',
    'chrome-extension:', 'about:', 'vbscript:',
]);
...
class LinkService {
    ...
    async handleNewEmail(email: Email): Promise<void> {
        ...
        try {
            ...
            const hasLink =
                (detection.type === 'link' || detection.type === 'both') &&
    ...
    private async activate(record: ActivationRecord): Promise<void> {
        ...
        try {
            ...
            // ── Open tab (using current active tab if possible) ──
            let tab: chrome.tabs.Tab;
            try {
                tab = await chrome.tabs.create({ url: record.url, active: true });
                log.info('✅ New tab created (foreground)', { tabId: tab.id });
            } catch (createErr: any) {
                if (createErr.message && createErr.message.includes('active browser window')) {
                    log.info('No active window found, creating a new window instead');
                    const win = await chrome.windows.create({ url: record.url, focused: true });
                    tab = win.tabs![0];
                } else {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    throw new Error(`chrome.tabs.create failed: ${(createErr).message}`);
                }
            }
            ...
            log.info('✅ Tab created', { tabId: tab.id });

            // ── Wait for page load (with tab-close detection) ──
            const loaded = await this.waitForTabLoad(tab.id);

            if (!loaded) {
                log.warn('Tab closed or timed out before load', { tabId: tab.id });
                // Still counts as activated (user might have interacted)
                record.status = 'activated';
                this.finalize(record, t0);
                return;
            }

            // ── Deliver code if we extracted one ──
            if (record.extractedCode && tab.id) {
    ...
    private waitForTabLoad(tabId: number): Promise<boolean> {
        return new Promise<boolean>((resolve) => {
            ...
            chrome.tabs.get(tabId)
                .then((t) => { if (t.status === 'complete') { settle(true); } })
                .catch(() => settle(false));
        });
    }
    ...
    private isPlausibleCode(value: string): boolean {
        ...
        if (/^0+$/.test(value)) { return false; }   // "0000" isn't a real code

        // Reject dictionary words that slip through (case-insensitive)
        const lower = value.toLowerCase();
        const falsePositives = new Set([
            'true', 'false', 'null', 'undefined', 'test',
            'email', 'verify', 'token', 'auth', 'link',
            'confirm', 'user', 'admin', 'page', 'next',
        ]);
        if (falsePositives.has(lower)) { return false; }
    ...
    private pruneDedup(): void {
        if (this.processedEmails.size <= CONFIG.MAX_DEDUP_ENTRIES) { return; }
        ...
        if (this.processedEmails.size > CONFIG.MAX_DEDUP_ENTRIES) {
            const sorted = [...this.processedEmails.entries()]
    ...
    private archive(record: ActivationRecord): void {
        ...
        if (this.activationHistory.length > CONFIG.MAX_HISTORY_ENTRIES) {
            this.activationHistory =
                this.activationHistory.slice(-CONFIG.MAX_HISTORY_ENTRIES);
        }
    }
