...
export function isValidUrl(url: string): boolean {
    ...
    const invalidSchemes = /^(mailto:|tel:|sms:|#|javascript:|data:|blob:|file:|chrome:|about:|cid:|ftp:)/i;
    if (invalidSchemes.test(url)) {return false;}
    if (!/^https?:\/\//i.test(url)) {return false;}

    try {
        const parsed = new URL(url);
        if (!parsed.hostname.includes('.') && parsed.hostname !== 'localhost') {return false;}
        if (/\.(png|jpg|jpeg|gif|svg|ico|woff2?|ttf|eot|css|webp|avif|bmp|tiff?|js)(\?|$)/i.test(parsed.pathname)) {return false;}
        return true;
    } catch {
        return false;
    }
...
export function normalizeUrl(url: string): string {
    const junkParams = [
        ...
        'fbclid', 'gclid', 'mc_cid', 'mc_eid', 'ref', '_hsenc', '_hsmi',
        'trk', 'trkCampaign', 'sc_channel', 'sc_campaign', 'sc_content',
        'mkt_tok', 'vero_id', 'oly_enc_id', 'oly_anon_id',
    ];
...
const FOOTER_SIGNALS = [
    /unsubscribe/i, /email preferences/i, /privacy policy/i,
    /©\s*\d{4}/i, /all rights reserved/i, /no longer wish to receive/i,
    /manage.*(?:preferences|subscription)/i, /this email was sent/i,
    /you are receiving this/i, /update your preferences/i,
    /view in browser/i, /view this email/i, /<footer/i,
    /class\s*=\s*["'][^"']*footer[^"']*["']/i,
];
