...
const STATIC_RESOURCE_PATTERN = /\.(png|jpg|jpeg|gif|svg|ico|woff2?|ttf|eot|css|webp|avif|bmp|tiff?|js)(\?|$)/i;

/** Invalid URL schemes to exclude */
const INVALID_URL_SCHEMES = /^(mailto:|tel:|sms:|#|javascript:|data:|blob:|file:|chrome:|about:|cid:|ftp:)/i;
...
export function isValidUrl(url: string): boolean {
    ...
    if (url.length < CONFIG.limits.minUrlLength) { return false; }
    if (INVALID_URL_SCHEMES.test(url)) { return false; }
    if (!/^https?:\/\//i.test(url)) { return false; }

    try {
        const parsed = new URL(url);
        if (!parsed.hostname.includes('.') && parsed.hostname !== 'localhost') { return false; }
        if (STATIC_RESOURCE_PATTERN.test(parsed.pathname)) { return false; }
        return true;
    } catch {
        return false;
    }
...
export function extractUrls(html: string): string[] {
    ...
    for (const url of [...urls]) {
        try {
            ...
            if (TRACKING_DOMAIN_PATTERNS.some((p) => p.test(u.hostname))) {
                ...
                if (inner && inner !== url) { urls.add(inner); }
    ...
    if (urls.size === 0) {
        console.warn('Extracted 0 unique URLs. RAW HTML START:\n' + html.substring(0, 1500) + '\nRAW HTML END');
    } else {
...
const TOKEN_PARAMS = ['token', 'auth_token', 'access_token', 'jwt', 'session', 'hash', 'key', 'secret', 'nonce', 'bearer', 'auth', 'id_token', 'state', 'flow'];
const CODE_PARAMS = ['code', 'confirmation_code', 'verify_code', 'auth_code', 'otp', 'response_code', 'activation_code', 'link_code'];
...
export function analyzeUrlParams(url: string): {
    ...
    try {
        ...
        for (const [k, v] of u.searchParams) {
            ...
            if (CODE_PARAMS.some((c) => lk.includes(c))) { hasCode = true; }
            if (EXPIRY_PARAMS.some((e) => lk.includes(e))) { hasExpiry = true; }
            if (SIG_PARAMS.some((s) => lk.includes(s))) { hasSignature = true; }
            if (USER_PARAMS.some((up) => lk.includes(up))) { hasUserId = true; }
        }
...
export function calculateUrlComplexity(url: string): number {
    ...
    try {
        ...
        for (const seg of u.pathname.split('/')) {
            if (/^[A-Za-z0-9_-]{15,}$/.test(seg)) { complexity += 15; }
        }
        if (u.hash.length > 1) { complexity += 10; }
    } catch {
        // Skip invalid URLs
    }
