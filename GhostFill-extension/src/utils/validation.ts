...
export const userSettingsSchema = z.object({
    ...
    checkIntervalSeconds: safeNumber.min(3).max(60).default(30),
...
export const generateEmailPayloadSchema = z.object({
    prefix: safeString.optional(),
    domain: safeString.optional(),
    service: emailServiceSchema.optional(),
}).optional();

export const checkInboxPayloadSchema = z.object({
...
export const fillOTPPayloadSchema = z.object({
    otp: safeString.regex(/^\d{4,10}$/, 'OTP must be 4-10 digits'),
    fieldSelectors: z.array(safeString).max(MAX_ARRAY_LENGTH),
});

export const otpPageDetectedPayloadSchema = z.object({
    priority: safeNumber.min(0).max(10).optional(),
    hostname: safeString.optional(),
});
...
export function validateMessage<T extends { action: string; payload?: unknown }>(
    ...
    try {
        ...
        if (payloadSchema !== z.undefined()) {
            payloadSchema.parse(message.payload);
        } else if (message.payload !== undefined && message.payload !== null) {
            return {
                valid: false,
                error: `Action ${message.action} should not have a payload`,
            };
        }
