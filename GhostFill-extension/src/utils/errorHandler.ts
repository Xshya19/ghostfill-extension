...
/**
 * Create a standardized error response for message handlers
 */
export function toErrorResponse(error: unknown, context?: string): { success: false; error: string } {
...
/**
 * Create a standardized success response for message handlers
 */
export function toSuccessResponse<T>(data?: T): { success: true } & (T extends undefined ? Record<string, never> : T) {
    if (data === undefined) {
        return { success: true } as { success: true } & (T extends undefined ? Record<string, never> : T);
    }
    return { success: true, ...data } as { success: true } & (T extends undefined ? Record<string, never> : T);
}
