...
export async function safeSendMessage(
    ...
    if (!validation.valid) {
        log.error('Message validation failed', { action: message.action, error: JSON.stringify(validation.error) });
...
export async function safeSendTabMessage(
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        let timeoutId: ReturnType<typeof setTimeout> | undefined;
        try {
            // Create timeout promise with cleanup (same pattern as safeSendMessage)
            const timeoutPromise = new Promise<null>((_, reject) => {
                timeoutId = setTimeout(() => reject(new Error(`Tab message timeout after ${timeout}ms`)), timeout);
            });
