...
export function extractOTP(text: string): string | null {
    ...
    const otpKeywords = [
        ...
        'enter code', 'use code', 'type code', 'input code',
        'login code', 'sign in code', 'access code', '2fa code',
        'password reset', 'recovery code', 'code to verify', 'code for',
    ];
    ...
    const scoredCandidates = candidates
        .filter(({ value, context }) => {
            ...
            if (/(?:order|tracking|shipment|package|delivery|fedex|ups|usps|dhl)\s*(?:#|number|no)/i.test(context)) {
                return false;
            }

            // Check for reference/invoice context
            if (/(?:reference|ref|ticket|case|invoice|receipt|transaction)\s*(?:#|number|no|id)/i.test(context)) {
                return false;
            }
...
export function extractActivationLink(text: string): string | null {
    ...
    const knownProviderDomains = [
        ...
        'microsoft.com', 'live.com', 'outlook.com', 'account.microsoft.com',
    ...
    const tokenParamPatterns = [
        '[?&]t=', '[?&]token=', '[?&]key=', '[?&]code=',
        '[?&]auth=', '[?&]access_token=', '[?&]id_token=',
        '[?&]verification=', '[?&]verify=', '[?&]confirm=',
        '[?&]activation=', '[?&]activate=', '[?&]v=',
        '[?&]hash=', '[?&]sig=', '[?&]signature=',
        '[?&]uuid=', '[?&]uid=', '[?&]user=',
        '[?&]flow=', '[?&]oobcode=', '[?&]continue=',
    ];

    // Verification-related URL keywords (original patterns)
    const verificationKeywords = [
        'verify', 'confirm', 'activate', 'token', 'auth', 'click',
        'register', 'validate', 'approve', 'accept', 'complete', 'signup',
        'signin', 'sign-in', 'login', 'log-in', 'password-reset',
        'email-verify', 'account-verify', 'two-factor', '2fa',
    ];
