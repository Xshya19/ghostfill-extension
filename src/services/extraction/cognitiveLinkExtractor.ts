...
const ACTION_ANCHOR_RE = /\b(verify|confirm|activate|reset|get started|accept invite|sign in|log in|continue|authorize|approve|join)\b/i;
const GENERIC_FOOTER_RE = /\b(unsubscribe|preferences|privacy|terms|copyright|view in browser)\b/i;
...
export function extractLinkCognitive(
  ...
  interface Cand { url: string; points: number; type: EmailIntent; anchorText: string; isCTA: boolean; originBound: boolean; mentions: number; }
  ...
  for (const rawUrl of urls) {
    ...
    try {
      ...
      for (const [re, score, type] of PATH_SCORE_MAP) {
        if (re.test(path)) { points += score; detectedType = type; break; }
      }

      const paramAn = analyzeUrlParams(workingUrl);
      if (paramAn.hasToken) points += 20;
      if (paramAn.hasCode) points += 12;
      if (paramAn.tokenLength > 30) points += 10;
      if ((paramAn.hasToken || paramAn.hasCode) && detectedType === 'other') detectedType = 'activation';
      ...
      if (expectedDomains.length > 0) {
        ...
        if (bound) points += 35; else points -= 22;
      }
    } catch { continue; }

    // Anchor-based signal via REAL DOM lookup (exact + fallback match).
    ...
    if (GENERIC_FOOTER_RE.test(anchorText)) points -= 50;
    else if (ACTION_ANCHOR_RE.test(anchorText)) points += 25;
    if (isCTA) points += 18;

    if (intent.intent === detectedType) points += 18;

    const existing = byUrl.get(normalized);
    const originBound = expectedDomains.length > 0 && points > 30; // recomputed below more precisely if needed
    if (existing) {
      existing.mentions += 1;
      existing.points = Math.max(existing.points, points) + 15; // multi-mention agreement bonus
      if (anchorText) existing.anchorText = existing.anchorText || anchorText;
      existing.isCTA = existing.isCTA || isCTA;
    } else {
      byUrl.set(normalized, {
        url: normalized, points, type: detectedType, anchorText, isCTA,
        originBound, mentions: 1,
      });
    }
  }
  ...
  const sorted = [...byUrl.values()].sort((a, b) => b.points - a.points);
  const best = sorted[0]!;

  log.info(`Cognitive Link: ${best.url.substring(0, 70)}... points=${best.points.toFixed(0)} mentions=${best.mentions} CTA=${best.isCTA}`);

  return {
    url: best.url,
    score: Math.min(Math.max(best.points, 0), 100),
    confidence: Math.min(Math.max(best.points, 0), 100),
    type: best.type,
    hasEmbeddedCode: false,
    embeddedCode: null,
    embeddedCodeParam: null,
    anchorText: best.anchorText,
    context: best.anchorText,
    domainTrust: best.originBound ? 90 : 50,
