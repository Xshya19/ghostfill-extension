...
import { StorageSchema, UserSettings, DEFAULT_SETTINGS, STORAGE_KEYS, SessionSecrets } from '../types';
import { deepMerge } from '../utils/core';
import { encrypt, decrypt, generateSecurePassword, initializeSecureEncryption, getSessionKey, clearEncryptionKeys } from '../utils/encryption';  // eslint-disable-line @typescript-eslint/no-unused-vars
...
class StorageService {
    ...
    // CRITICAL FIX #2: Optimistic UI - pending optimistic updates
    ...
    constructor() {
        this.cache = new LRUCache<keyof StorageSchema, unknown>(CACHE_CONFIG.MAX_SIZE, CACHE_CONFIG.TTL_MS);
    }
     ...
     */
    private async acquireWriteMutex(): Promise<void> {
    ...
    async setOptimistic<K extends keyof StorageSchema>(
        ...
        setTimeout(() => {
            this.set(key, value).then(() => {
                ...
                log.error(`Optimistic update failed for ${key}`, error);
                this.optimisticUpdates.delete(key);
            });
        }, syncDelay);
     ...
     */
    async getWithOptimistic<K extends keyof StorageSchema>(key: K): Promise<StorageSchema[K] | undefined> {
     ...
     */
    setSessionSecret<K extends keyof SessionSecrets>(key: K, value: SessionSecrets[K]): void {
    ...
    setLLMApiKey(apiKey: string): void {
        ...
        this.setSessionSecret('llmApiKey', apiKey);
        this.setSessionSecret('keyRotatedAt', Date.now());
    }
    ...
    setCustomDomainKey(apiKey: string): void {
        ...
        this.setSessionSecret('customDomainKey', apiKey);
        this.setSessionSecret('keyRotatedAt', Date.now());
    }
        ...
        try {
            ...
            if (value && SENSITIVE_KEYS.includes(key) && typeof value === 'string') {
                try {
                    ...
                    log.warn(`Failed to decrypt ${key}, returning as-is`, error);
                }
            }
    ...
    private async setInternal<K extends keyof StorageSchema>(key: K, value: StorageSchema[K]): Promise<void> {
    ...
    private async flushPendingWrites(): Promise<void> {
        ...
        if (this.isFlushing) {
            log.debug('Flush already in progress, waiting for queue');
            ...
            try {
                ...
                for (const [key, value] of writes.entries()) {
                    ...
                    if (usage.percentage >= this.QUOTA_WARNING_THRESHOLD * 100) {
                        log.error(`Storage quota at ${usage.percentage.toFixed(1)}% - refusing write`, { key, size: valueSize });
                        this.pendingWrites.set(key, value);
                    ...
                    if (SENSITIVE_KEYS.includes(key as keyof StorageSchema)) {
                        ...
                        if (sessionKey) {
                            try {
                                valueToStore = await encrypt(value, this.getEncryptionKey());
                            } catch (error) {
                ...
                // BUG FIX: Don't call setImmediate() which would call flushPendingWrites() again
                // Instead, schedule a retry directly if there are pending writes
                if (this.pendingWrites.size > 0) {
                    log.debug('Scheduling retry for pending writes', { count: this.pendingWrites.size });
                    // BUG FIX: Directly schedule retry instead of calling setImmediate to avoid double flush
                    setTimeout(() => {
        ...
        this.writeQueue = scheduledWrite.catch(() => { });
     ...
     */
    async setImmediate<K extends keyof StorageSchema>(key: K, value: StorageSchema[K]): Promise<void> {
        await this.ensureInitialized();
        ...
        this.pendingWrites.set(key, value);
        this.cache.set(key as keyof StorageSchema, value);
        // BUG FIX: Check if already flushing to avoid recursive flush
        if (this.isFlushing) {
            return this.writeQueue;
        }
    ...
    private async getAllInternal(): Promise<Partial<StorageSchema>> {
        try {
            ...
            for (const [key, value] of Object.entries(result)) {
                ...
                if (value && SENSITIVE_KEYS.includes(key as keyof StorageSchema) && typeof value === 'string') {
                    try {
                        const sessionKey = getSessionKey();
                        if (sessionKey) { // we might be inside initialization, so check
                            finalValue = await decrypt(value as string, sessionKey as any);
                        }
                    } catch (error) {
                        log.warn(`Failed to decrypt ${key} in getAllInternal, keeping as-is`, error);
                    }
                }
    ...
    private async pruneOldData(): Promise<void> {
        try {
            ...
            if (emailHistory && Array.isArray(emailHistory) && emailHistory.length > 20) {
                await this.setImmediate('emailHistory', emailHistory.slice(0, 20) as StorageSchema['emailHistory']);
            ...
            if (passwordHistory && Array.isArray(passwordHistory) && passwordHistory.length > 20) {
                await this.setImmediate('passwordHistory', passwordHistory.slice(0, 20) as StorageSchema['passwordHistory']);
            ...
            if (areaName === 'local') {
                ...
                for (const key in changes) {
                    if (changes[key].newValue !== undefined) {
                        this.cache.set(key as keyof StorageSchema, changes[key].newValue);
                    } else {
