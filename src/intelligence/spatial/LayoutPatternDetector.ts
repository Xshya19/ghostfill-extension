...
export type LayoutPattern = 
  | 'vertical_login' | 'vertical_signup' | 'horizontal_otp' 
  | 'split_screen' | 'modal_overlay' | 'inline_form' | 'unknown';
...
export class LayoutPatternDetector {
  ...
  public static detectClusters(): FormCluster[] {
    ...
    const inputs = Array.from(document.querySelectorAll('input:not([type="hidden"]), select, textarea, button'));
    ...
    for (const input of inputs) {
      ...
      const clusterElements = instance.findNearbyElements(input as HTMLElement, inputs as HTMLElement[]);
      ...
      if (clusterElements.length > 0) {
        ...
        clusters.push({ elements: clusterElements, boundingBox, layoutPattern: pattern, confidence });
      }
    }
  ...
  private findNearbyElements(start: HTMLElement, all: HTMLElement[]): HTMLElement[] {
    ...
    for (const el of all) {
      ...
      const dist = Math.sqrt(Math.pow(startRect.left - rect.left, 2) + Math.pow(startRect.top - rect.top, 2));
      if (dist < threshold) {cluster.push(el);}
    }
    return cluster;
  }
  ...
  private classifyPattern(elements: HTMLElement[], box: DOMRect): { pattern: LayoutPattern; confidence: number } {
    ...
    if (inputs.length >= 4 && inputs.length <= 8) {
      const isHorizontal = inputs.every(el => Math.abs(el.getBoundingClientRect().top - inputs[0].getBoundingClientRect().top) < 10);
      if (isHorizontal) {return { pattern: 'horizontal_otp', confidence: 0.9 };}
    }

    // 2. Detect Vertical Login
    if (inputs.length >= 2 && inputs.length <= 3 && buttons.length >= 1) {
      const isVertical = inputs.every((el, i) => i === 0 || el.getBoundingClientRect().top > inputs[i-1].getBoundingClientRect().top);
      if (isVertical && box.width < 500) {return { pattern: 'vertical_login', confidence: 0.85 };}
    }

    // 3. Detect Modal Overlay
    const modal = elements[0].closest('[role="dialog"], .modal, .overlay');
    if (modal) {return { pattern: 'modal_overlay', confidence: 0.95 };}
