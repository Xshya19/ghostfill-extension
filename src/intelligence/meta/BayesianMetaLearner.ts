...
import {
  MetaLearnerState, DomainProfile, DetectionLayer, ALL_LAYERS,
} from '../../types/sentinel';
...
export class BayesianMetaLearner {
  ...
  static async fuse(
    ...
    for (const cls of FIELD_CLASSES) {fusedScores[cls] = 0;}
    ...
    const predictions = activeLayers.map(l => {
        ...
        for (const [c, s] of Object.entries(output)) {
            if ((s as number) > max) { max = s as number; best = c; }
        }
        return best;
    });
    
    const uniquePreds = new Set(predictions).size;
    const disagreement = activeLayers.length > 1 ? (uniquePreds - 1) / (activeLayers.length - 1) : 0;
  ...
  static async recordOutcome(domain: string, layeredResults: Record<DetectionLayer, number>, success: boolean): Promise<void> {
