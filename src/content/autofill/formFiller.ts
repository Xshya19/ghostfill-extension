...
// ─── UniversalFiller & Strategies ────────────────────────────────────

export interface StrategyFillResult {
  success: boolean;
  strategy: string;
}
...
export class AngularZoneStrategy extends FillStrategy {
  name = 'angular-zone';
  ...
  async execute(element: HTMLInputElement | HTMLTextAreaElement, value: string): Promise<boolean> {
    try {
      ...
      if (ng && typeof ng.getComponent === 'function') {
        ...
        if (comp && typeof ng.applyChanges === 'function') {
...
export class SvelteStrategy extends FillStrategy {
  name = 'svelte';
...
export class SolidJSStrategy extends FillStrategy {
  name = 'solid-js';
...
export class ExecCommandStrategy extends FillStrategy {
  name = 'exec-command';
