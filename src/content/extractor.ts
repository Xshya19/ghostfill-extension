...
function analyzeSubmitButton(el: HTMLElement): {
  dist: number;
  actionText: string;
  formAction: string;
} {
  ...
  const buttons = form.querySelectorAll(
    'button, input[type="submit"], [role="button"], a[class*="btn"]'
  );
  ...
  buttons.forEach((btn) => {
    ...
    if (dist < closestDist) {
      closestDist = dist;
      closestText = ((btn as HTMLElement).textContent || (btn as HTMLInputElement).value || '')
        .trim()
        .toLowerCase();
    }
  });
...
function detectSplitOTP(el: HTMLInputElement | HTMLTextAreaElement): OTPGroupInfo {
  ...
  const potentialSiblings = Array.from(root.querySelectorAll<HTMLInputElement>('input')).filter(
    (inp) => {
      if (inp.maxLength !== maxLen) {
        return false;
      }
...
export function extractFeatures(el: HTMLInputElement | HTMLTextAreaElement): RawFieldFeatures {
  ...
  vec[21] = /\b(log[_-]?in|sign[_-]?in|authenticate|connexion|iniciar[_-]?sesión)\b/i.test(
    submitInfo.actionText
  )
    ? 1
    : 0;
  vec[22] = /\b(sign[_-]?up|register|create[_-]?account|join|inscription|registrarse)\b/i.test(
    submitInfo.actionText
  )
    ? 1
    : 0;
  vec[23] = /\b(submit|continue|next|verify|confirm|send|go|enter|proceed)\b/i.test(
    submitInfo.actionText
  )
    ? 1
    : 0;
  ...
  vec[27] =
    /\b(user[-_]?name|login[-_]?id|screen[-_]?name|handle|acct[-_]?name|identifiant|usuario)\b/i.test(
      allText
    )
      ? 1
      : 0;
  vec[28] = /\b(pass[-_]?word|pwd|contraseña|mot[-_]?de[-_]?passe|passwort|senha)\b/i.test(allText)
    ? 1
    : 0;
  vec[29] =
    /\b(confirm[-_]?pass|re[-_]?(?:enter|type)[-_]?pass|verify[-_]?pass|repeat[-_]?pass|pass[-_]?confirm|password[-_]?again)\b/i.test(
      allText
    )
      ? 1
      : 0;
  vec[30] = /\b(first[-_]?name|given[-_]?name|f[-_]?name|prénom|nombre|vorname)\b/i.test(allText)
    ? 1
    : 0;
  vec[31] =
    /\b(last[-_]?name|sur[-_]?name|family[-_]?name|l[-_]?name|nom[-_]?(?:de[-_]?)?famille|apellido|nachname)\b/i.test(
      allText
    )
      ? 1
      : 0;
  vec[32] =
    /\b(full[-_]?name|your[-_]?name|name|display[-_]?name|nom[-_]?complet|nombre[-_]?completo)\b/i.test(
      allText
    )
      ? 1
      : 0;
  vec[33] = /\b(phone|tel(?:ephone)?|mobile|cell|número|numéro|telefon|sms[-_]?number)\b/i.test(
    allText
  )
    ? 1
    : 0;
  vec[34] =
    /\b(otp|verif(?:y|ication)[-_]?code|security[-_]?code|one[-_]?time|auth(?:entication)?[-_]?code|pin[-_]?code|mfa[-_]?code|2fa[-_]?code|token|passcode|code[-_]?sent)\b/i.test(
      allText
    )
      ? 1
      : 0;
  ...
  vec[43] =
    sibInfo.distanceToPrevPassword >= 0 ? Math.min(sibInfo.distanceToPrevPassword, 5) / 5.0 : 0;
  ...
  vec[47] = Math.min(
    (el.id.includes('otp') || el.name.includes('otp') ? 0.5 : 0) +
      (sibInfo.formFieldCount < 3 ? 0.3 : 0),
    1.0
  );
...
export function collectAllFields(root: Document | ShadowRoot = document): RawFieldFeatures[] {
  ...
  for (const el of Array.from(
    root.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>(COLLECT_SELECTOR)
  )) {
