...
class KeyMapper {
  ...
  static getCode(char: string): string {
    ...
    // Special characters
    return this.SPECIAL_CHARS[char] ?? 'Unidentified';
  }
...
export class PhantomTyper {
  ...
  static async typeSimulatedString(
    ...
    try {
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        // Minimal yield to allow framework digest cycles
        if (i < text.length - 1) {
          await this.yield(INTER_CHAR_YIELD_MS);
        }
      }
  ...
  private static typeCharacter(
    ...
    // If beforeinput was cancelled, skip value change
    // (some frameworks use this for input masking/filtering)
    if (!allowed) {return;}
