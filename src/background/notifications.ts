...
type NotificationCategory = 'otp' | 'email' | 'link' | 'success' | 'error' | 'system';
...
type ButtonActionHandler = (notificationId: string, data: Record<string, unknown>) => Promise<void>;

type ClickActionHandler = (notificationId: string, data: Record<string, unknown>) => Promise<void>;
...
export async function notifyLinkActivated(url: string, code?: string): Promise<string> {
  return notify({
    ...
    message: code ? `Code ${maskOTP(code)} auto-filled!` : `Activation link opened in new tab.`,
    priority: 1,
    data: { url, code },
  });
}
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    try {
      chrome.notifications.create(id, options, (notifId) => {
        ...
        const autoClearMs =
          spec.autoClearMs !== undefined ? spec.autoClearMs : catSettings.autoClearMs;
...
async function getCategorySettings(category: NotificationCategory): Promise<CategorySettings> {
  try {
    const settings = await storageService.getSettings();
    const overrides = (
      settings as { notificationCategories?: Record<string, Partial<CategorySettings>> }
...
function maybePruneDedup(force: boolean = false): void {
  const now = Date.now();
  if (!force && now - lastDedupPruneAt < CONFIG.DEDUP_PRUNE_INTERVAL) {
    return;
  }
...
function pruneDedup(): void {
  ...
  if (dedupCache.size > CONFIG.MAX_DEDUP_ENTRIES) {
    const sorted = Array.from(dedupCache.entries()).sort(
...
function isCategoryRateLimited(category: NotificationCategory, maxPerMinute: number): boolean {
  const now = Date.now();
