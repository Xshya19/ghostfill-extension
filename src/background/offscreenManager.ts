...
type DocState = 'closed' | 'creating' | 'alive' | 'errored' | 'closing';
...
class OffscreenManager {
  ...
  async ensureDocument(reason?: chrome.offscreen.Reason, justification?: string): Promise<void> {
   ...
   */
  async sendMessage<T = unknown>(action: string, payload?: Record<string, unknown>): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        ...
        reject(
          new Error(`Offscreen message "${action}" timed out after ${CONFIG.MESSAGE_TIMEOUT_MS}ms`)
        );
  ...
  private async createWithRetry(
    ...
    // SECURITY/RELIABILITY FIX: Map creation to a singleton Promise immediately.
    // This ensures the `documentExists` async wait doesn't permit identical concurrent executions.
        ...
        for (let attempt = 0; attempt <= CONFIG.MAX_CREATE_RETRIES; attempt++) {
          try {
            ...
            this.metrics.avgCreateMs =
              this.metrics.avgCreateMs === 0 ? ms : this.metrics.avgCreateMs * 0.8 + ms * 0.2;
            ...
            const backoff = Math.min(CONFIG.RETRY_BASE_MS * 2 ** attempt, CONFIG.RETRY_CAP_MS);
    ...
    try {
      await Promise.race([
        ...
        rejectAfter(15000, 'Offscreen creation timed out (15s limit reached)'),
      ]);
    } finally {
      // Only clear the promise if we actually transitioned away from 'creating'
  ...
  private async documentExists(): Promise<boolean> {
    ...
    try {
      if ('getContexts' in chrome.runtime) {
        ...
        log.debug('documentExists check via getContexts', {
          exists,
  ...
  private scheduleNextHealthCheck(): void {
    ...
    this.healthTimer = setTimeout(() => {
      this.runHealthCheck()
        .catch((err) => log.debug('Health check error', extractMsg(err)))
        .finally(() => {
          if (this.state === 'alive') {
            this.scheduleNextHealthCheck();
          }
  ...
  private async sendPing(): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      try {
        chrome.runtime.sendMessage({ target: 'offscreen', action: 'HEALTH_PING' }, (response) => {
  ...
  private onHealthFailure(reason: string): void {
    ...
    if (this.healthFailures >= CONFIG.HEALTH_FAILURES_MAX) {
      ...
      this.close().catch((e) => log.debug('Offscreen close failed', e));
    }
  }
  ...
  private installMessageListener(): void {
    ...
    chrome.runtime.onMessage.addListener((msg) => {
      ...
      if (msg?.source !== 'offscreen' || !msg?.id) {
        return false;
      }
