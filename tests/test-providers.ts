...
async function testProviders() {
  ...
  try {
    ...
    addResult('DropMail', acc.fullEmail ? 'OK' : 'WARN', `Generated: ${acc.fullEmail}`);
  } catch (e: any) {
    addResult('DropMail', 'FAIL', e.message);
  }
  try {
  ...
  try {
    const acc = await tempMailLolService.createAccount();
    addResult('TempMail.lol', acc.fullEmail ? 'OK' : 'WARN', `Generated: ${acc.fullEmail}`);
  } catch (e: any) {
    addResult('TempMail.lol', 'FAIL', e.message);
  }
  try {
    const acc = await tmailorService.createAccount('testrunner');
    addResult('TMailor', acc.fullEmail ? 'OK' : 'WARN', `Generated: ${acc.fullEmail}`);
  } catch (e: any) {
    addResult('TMailor', 'FAIL', e.message);
  }
